...
var et = (B, t, e) =>
  t in B
    ? tt(B, t, { enumerable: !0, configurable: !0, writable: !0, value: e })
    : (B[t] = e);
...
import { Z as T } from "./index.e3fb11d0.js";
...
class ft {
    ...
    ((this.canvas = t),
      ...
      (this.isMobile =
        typeof navigator < "u"
          ...
          : !1),
      (this.cols = Math.max(1, Math.floor(e.cols ?? 64))),
      (this.rows = Math.max(1, Math.floor(e.rows ?? 40))));
    const o = this.isMobile ? 1.5 : 3;
    ((this.dprCap = Math.max(1, e.dprCap ?? o)),
      this.isMobile && (this.dprCap = Math.min(this.dprCap, 1.5)),
      (this.targetCount = Math.max(1, Math.floor(e.count ?? 220))),
      (this.spawnBandX = Math.max(1, Math.floor(e.spawnBandX ?? 8))),
      (this.spawnBandY = Math.max(1, Math.floor(e.spawnBandY ?? 8))),
      (this.minSegments = Math.max(2, Math.floor(e.minSegments ?? 2))),
      (this.maxSegments = Math.max(
        this.minSegments,
        Math.floor(e.maxSegments ?? 3),
      )),
      (this.shapeBranchChance = this.clamp01(e.shapeBranchChance ?? 0.45)),
      (this.shapeDownBias = this.clamp01(e.shapeDownBias ?? 0.7)),
      (this.shapeMaxHorizontal = Math.max(
        1,
        Math.floor(e.shapeMaxHorizontal ?? 3),
      )),
      (this.windCurvePower = e.windCurvePower ?? 1),
      (this.windBaseStrength = e.windBaseStrength ?? -0.08),
      (this.windCurveScale = e.windCurveScale ?? 0.1),
      (this.windSineFreq = e.windSineFreq ?? Math.PI * 2),
      (this.windSineAmp = e.windSineAmp ?? 0.25),
      (this.vxRange = e.vxRange ?? [-1.8, -0.4]),
      (this.vyRange = e.vyRange ?? [0.25, 1.3]),
      (this.bgColor = e.bgColor ?? [0, 0, 0, 0]),
      (this.palette = e.palette ?? [
        [1, 0.4392, 0.3608, 1],
        [0.898, 0.3333, 0.2549, 1],
        [1, 0.3098, 0.2118, 1],
        [0.8863, 0.2392, 0.149, 1],
        [0.949, 0.1765, 0.0667, 1],
      ]),
      (this.introSpawnOutside = e.introSpawnOutside ?? !0),
      (this.introMarginX = Math.max(1, Math.floor(e.introMarginX ?? 6))),
      (this.introMarginY = Math.max(1, Math.floor(e.introMarginY ?? 6))),
      (this.introRampSec = Math.max(0.3, e.introRampSec ?? 1)),
      (this.spawnPerSec = Math.max(1, e.spawnPerSec ?? 60)),
      (this.minSpawnGridGap = Math.max(0, Math.floor(e.minSpawnGridGap ?? 2))),
      (this.rowCooldownLen = Math.max(0, Math.floor(e.rowCooldownLen ?? 3))),
      (this.colCooldownLen = Math.max(0, Math.floor(e.colCooldownLen ?? 2))),
      (this.introFanOutSec = Math.max(0, e.introFanOutSec ?? 0.6)),
      (this.introFanOutVX = e.introFanOutVX ?? 0.6),
      (this.introFanOutVY = e.introFanOutVY ?? 0.6),
      (this.cellMoveSpeedX = Math.max(0, e.cellMoveSpeedX ?? 1)),
      (this.cellMoveSpeedY = Math.max(0, e.cellMoveSpeedY ?? 1)),
      (this.maxInstances = Math.min(this.targetCount * this.maxSegments, Z)),
      (this.boundContextLost = (n) => {
      ...
      this.initGL(),
      this.expandInstanceStorage(Math.min(this.maxInstances, 256)),
      (this.rowCd = new Uint8Array(this.rows)),
      (this.colCd = new Uint8Array(
        this.cols + this.introMarginX + this.shapeMaxHorizontal + 2,
      )),
      (this.stamps = new Uint16Array(this.rows * this.cols)),
      (this.owners = new Uint16Array(this.rows * this.cols)),
      this.initSpatialBuckets(),
      this.initTeleportScratch(),
      this.rebuildWindLUT(),
  ...
  ensureBucketCapacity(t) {
    ...
    (e <= this.bucketCapacity && this.bucketNext) ||
      ((this.bucketNext = new Int32Array(e)),
      (this.bucketLeafIndex = new Int32Array(e)),
      (this.bucketCellCol = new Int16Array(e)),
      (this.bucketCellRow = new Int16Array(e)),
      (this.bucketCapacity = e),
      (this.bucketValid = !1),
      (this.bucketDirty = !0));
  }
  ...
  expandInstanceStorage(t) {
    ...
    for (
      o === 0 && (o = Math.max(64, Math.min(this.maxInstances, s)));
      o < s && o < this.maxInstances;
    )
      if (
        ((o = Math.min(
          this.maxInstances,
          o < 1024 ? o << 1 : Math.floor(o * 1.5),
        )),
    ...
    (this.instanceCapacity > 0 &&
      (n.set(this.gridCoords.subarray(0, this.instanceCapacity * 2)),
      i.set(this.colors.subarray(0, this.instanceCapacity * 4))),
      (this.gridCoords = n),
      (this.colors = i),
      this.gridBuffers.length === 0 &&
        ((this.gridBuffers = [e.createBuffer(), e.createBuffer()]),
        (this.colBuffers = [e.createBuffer(), e.createBuffer()])));
    const r = this.gridCoords.byteLength,
      c = this.colors.byteLength;
    for (let l = 0; l < this.gridBuffers.length; l++)
      (e.bindBuffer(e.ARRAY_BUFFER, this.gridBuffers[l]),
        e.bufferData(e.ARRAY_BUFFER, r, e.STREAM_DRAW),
        e.bindBuffer(e.ARRAY_BUFFER, this.colBuffers[l]),
        e.bufferData(e.ARRAY_BUFFER, c, e.STREAM_DRAW));
    ((this.instanceCapacity = o), this.ensureBucketCapacity(o));
    const a =
      this.activeBufferIndex >= 0
        ? Math.min(this.activeBufferIndex, this.gridBuffers.length - 1)
        : 0;
  ...
  setActiveBufferIndex(t) {
    ...
    const e =
      ((t % this.gridBuffers.length) + this.gridBuffers.length) %
      this.gridBuffers.length;
  ...
  rotateInstanceBuffer() {
    ...
    const t =
      this.activeBufferIndex < 0
        ? 0
        : (this.activeBufferIndex + 1) % this.gridBuffers.length;
  ...
  updateInstanceBufferBinding(t) {
    ...
    (e.bindVertexArray(this.vao),
      e.bindBuffer(e.ARRAY_BUFFER, this.gridBuffers[t]),
      e.enableVertexAttribArray(this.gridAttribLoc),
      e.vertexAttribPointer(this.gridAttribLoc, 2, e.SHORT, !1, 0, 0),
      e.vertexAttribDivisor(this.gridAttribLoc, 1),
      e.bindBuffer(e.ARRAY_BUFFER, this.colBuffers[t]),
      e.enableVertexAttribArray(this.colAttribLoc),
      e.vertexAttribPointer(this.colAttribLoc, 4, e.UNSIGNED_BYTE, !0, 0, 0),
      e.vertexAttribDivisor(this.colAttribLoc, 1),
  ...
  handleContextRestored() {
    ...
    ((this.gl = t),
      ...
      this.initGL(),
      (this.instanceCapacity = 0),
      (this.gridCoords = new Int16Array(0)),
    ...
    if ((this.expandInstanceStorage(Math.max(n, 64)), o > 0)) {
      ...
      i > 0 &&
        (this.gridCoords.set(e.subarray(0, i * 2)),
        this.colors.set(s.subarray(0, i * 4)));
    }
    ((this.targetCount = Math.max(1, Math.floor(this.targetCount * 0.75))),
      (this.spawnPerSec *= 0.75),
      (this.maxInstances = Math.min(this.targetCount * this.maxSegments, Z)),
      (this.qualityStep = Math.max(this.qualityStep, 1)),
      this.isMobile
        ? (this.dprCap = Math.max(1, Math.min(this.dprCap, 1.25)))
        : (this.dprCap = Math.max(1, Math.min(this.dprCap, 2))),
      this.ensureBucketCapacity(this.instanceCapacity),
      this.markSpatialIndexDirty(),
      this.rebuildWindLUT(),
      this.resize());
  }
  rebuildWindLUT() {
    ...
    for (let t = 0; t < this.rows; t++) {
      ...
      this.windBaseByRow[t] =
        this.windBaseStrength -
        Math.pow(e, this.windCurvePower) * this.windCurveScale;
    }
  }
  initTeleportScratch() {
    const t = Math.max(3, this.maxSegments);
    ((this.tmpCandidateCols = new Int16Array(t)),
      (this.tmpCandidateRows = new Int16Array(t)),
      (this.tmpCandidateScores = new Int32Array(t)));
  }
  initSpatialBuckets() {
    ...
    ((this.bucketMinCol = -t),
      (this.bucketMaxCol = this.cols + t),
      (this.bucketMinRow = -e),
      (this.bucketMaxRow = this.rows + e));
    const s = this.bucketMaxCol - this.bucketMinCol + 1,
      o = this.bucketMaxRow - this.bucketMinRow + 1;
    ((this.bucketCols = Math.max(1, Math.ceil(s / this.bucketSide))),
      (this.bucketRows = Math.max(1, Math.ceil(o / this.bucketSide))));
    const n = this.bucketCols * this.bucketRows;
    ((this.bucketHeads = new Int32Array(n)),
      this.bucketHeads.fill(-1),
      this.ensureBucketCapacity(Math.max(1, this.instanceCapacity)),
      (this.bucketValid = !1),
      (this.bucketDirty = !0));
  }
  ...
  ensureSpatialIndex() {
    (!this.bucketDirty && this.bucketValid) ||
      (this.bucketRebuildsThisFrame >= 2 && this.bucketValid) ||
  ...
  rebuildSpatialIndex() {
    ...
    for (let s = 0; s < this.leaves.length; s++) {
      ...
      for (let c = 0; c < o.cellCount && !(t >= e); c++) {
        ...
        if (d < 0 || d >= this.bucketCols || f < 0 || f >= this.bucketRows)
          continue;
        const p = f * this.bucketCols + d;
        ((this.bucketCellCol[t] = l),
          (this.bucketCellRow[t] = u),
          (this.bucketLeafIndex[t] = s),
          (this.bucketNext[t] = this.bucketHeads[p]),
          (this.bucketHeads[p] = t),
          t++);
      }
    }
    ((this.bucketValid = !0),
      ...
      (this.lastBucketRebuildFrame = this.frameId));
  }
  candidateExists(t, e, s) {
    for (let o = 0; o < t; o++)
      if (this.tmpCandidateCols[o] === e && this.tmpCandidateRows[o] === s)
        return !0;
    return !1;
  }
  insertCandidate(t, e, s, o, n) {
    ...
    if (o === 0)
      return (
        (this.tmpCandidateScores[0] = s),
        (this.tmpCandidateCols[0] = t),
        (this.tmpCandidateRows[0] = e),
        1
      );
    if (o < n) {
      let r = o;
      ...
      for (let c = o; c > r; c--)
        ((this.tmpCandidateScores[c] = this.tmpCandidateScores[c - 1]),
          (this.tmpCandidateCols[c] = this.tmpCandidateCols[c - 1]),
          (this.tmpCandidateRows[c] = this.tmpCandidateRows[c - 1]));
      return (
        (this.tmpCandidateScores[r] = s),
        (this.tmpCandidateCols[r] = t),
        (this.tmpCandidateRows[r] = e),
        o + 1
      );
    }
    if (s >= this.tmpCandidateScores[n - 1]) return o;
    let i = n - 1;
    for (; i > 0 && s < this.tmpCandidateScores[i - 1];)
      ((this.tmpCandidateScores[i] = this.tmpCandidateScores[i - 1]),
        (this.tmpCandidateCols[i] = this.tmpCandidateCols[i - 1]),
        (this.tmpCandidateRows[i] = this.tmpCandidateRows[i - 1]),
        i--);
    return (
      (this.tmpCandidateScores[i] = s),
      (this.tmpCandidateCols[i] = t),
      (this.tmpCandidateRows[i] = e),
      o
    );
  }
  ...
  resize() {
    ...
    (l.useProgram(this.program),
      l.uniform2f(this.uCellSizeLoc, this.cwWorld, this.chWorld),
      l.uniform2f(this.uOffsetLoc, this.xOffWorld, this.yOffWorld),
      this.rebuildWindLUT(),
      (this.bucketDirty = !0));
  }
  setCellMoveSpeed(t, e) {
    (typeof t == "number" && isFinite(t) && (this.cellMoveSpeedX = t),
      typeof e == "number" && isFinite(e) && (this.cellMoveSpeedY = e));
  }
  clear() {
    const t = this.gl;
    (t.clearColor(
  ...
  attemptMicroRebuildX(t, e, s, o = 1) {
    ...
    if (t.cellCount === 1) {
      const n = t.cells[0] + s,
        i = t.cells[1];
      return this.canPlaceCellAtOffset(t, e, n, i)
        ? ((t.cells[0] = n),
          this.markBoundsDirty(t),
          this.normalizeLeafOffsets(t),
          (t.lastMoveX = s),
          (t.cooldownX = 2),
          this.markSpatialIndexDirty(),
          !0)
    ...
    if (t.cellCount === 2) {
      ...
      let C = Math.abs(p - d),
        m = Math.abs(x - f),
        w = Math.max(C, m) === 1;
      return w && this.canPlaceCellAtOffset(t, e, d, f, l)
        ? ((t.cells[l * 2] = d),
          (t.cells[l * 2 + 1] = f),
          this.markBoundsDirty(t),
          this.normalizeLeafOffsets(t),
        ...
        : ((d = t.cells[l * 2]),
          ...
          (m = Math.abs(x - f)),
          (w = Math.max(C, m) === 1),
          w && this.canPlaceCellAtOffset(t, e, d, f, l)
            ? ((t.cells[l * 2] = d),
              (t.cells[l * 2 + 1] = f),
              this.markBoundsDirty(t),
              this.normalizeLeafOffsets(t),
  ...
  attemptMicroRebuildY(t, e, s, o = 1) {
    ...
    if (t.cellCount === 2) {
      ...
      let p = Math.abs(d - l),
        x = Math.abs(f - u),
        C = Math.max(p, x) === 1;
      return C && this.canPlaceCellAtOffset(t, e, l, u, a)
        ? ((t.cells[a * 2] = l),
          (t.cells[a * 2 + 1] = u),
          this.markBoundsDirty(t),
          this.normalizeLeafOffsets(t),
        ...
        : ((l = t.cells[a * 2] + o),
          ...
          (x = Math.abs(f - u)),
          (C = Math.max(p, x) === 1),
          C && this.canPlaceCellAtOffset(t, e, l, u, a)
            ? ((t.cells[a * 2] = l),
              (t.cells[a * 2 + 1] = u),
              this.markBoundsDirty(t),
              this.normalizeLeafOffsets(t),
  ...
  update(t) {
    ...
    if (
      ((this.lastMs = t),
      !isFinite(e) || e <= 0 || ((e = Math.min(e, 1 / 60)), this.contextLost))
    )
      return;
    for (
      this.frameId++,
        this.bucketRebuildsThisFrame = 0,
        this.fpsAccum += e,
        this.fpsFrames++,
        this.fpsAccum >= 2 &&
          (this.fpsFrames / this.fpsAccum < 45 &&
            this.qualityStep < 2 &&
            (this.qualityStep++,
            (this.targetCount = Math.max(
              1,
              Math.floor(this.targetCount * 0.75),
            )),
            (this.spawnPerSec *= 0.75),
            (this.maxInstances = Math.min(
              this.targetCount * this.maxSegments,
              Z,
            )),
            this.leaves.length > this.targetCount &&
              ((this.leaves.length = this.targetCount),
              this.markSpatialIndexDirty()),
            this.isMobile
              ? (this.dprCap = Math.max(1, this.dprCap * 0.9))
              : (this.dprCap = Math.max(1, this.dprCap * 0.95)),
            (this.bucketDirty = !0),
            this.resize()),
          (this.fpsAccum = 0),
          (this.fpsFrames = 0)),
        this.currentStamp = (this.currentStamp + 1) & 65535,
        this.currentStamp === 0 &&
          ((this.currentStamp = 1), this.stamps.fill(0)),
        this.spawnAcc += e * this.spawnPerSec;
      this.leaves.length < this.targetCount && this.spawnAcc >= 1;
    )
      (this.trySpawnOne(), (this.spawnAcc -= 1));
    for (let n = 0; n < this.rowCd.length; n++)
      this.rowCd[n] > 0 && this.rowCd[n]--;
    for (let n = 0; n < this.colCd.length; n++)
      this.colCd[n] > 0 && this.colCd[n]--;
    let s = 0;
    for (let n = 0; n < this.leaves.length; n++) {
      ...
      ((i.phase += this.windSineFreq * e),
        i.phase >= N ? (i.phase -= N) : i.phase < 0 && (i.phase += N));
      ...
      ((i.xAccum += p),
        !w &&
          i.xAccum <= -(1 + k) &&
          (this.attemptMicroRebuildX(i, r, -1, m) ||
            this.teleportLeafHorizontal(i, r, -1)) &&
          ((i.xAccum += 1),
          i.xAccum > -v && (i.xAccum = -v),
          (w = !0),
          (g = -1)),
        !w &&
          i.xAccum >= 1 + k &&
          (this.attemptMicroRebuildX(i, r, 1, m) ||
            this.teleportLeafHorizontal(i, r, 1)) &&
          ((i.xAccum -= 1), i.xAccum < v && (i.xAccum = v), (w = !0), (g = 1)),
        (i.yAccum += x),
        !w &&
          i.yAccum >= 1 + k &&
          (this.attemptMicroRebuildY(i, r, 1, C) ||
            this.advanceLeafTeleport(i, r)) &&
          ((i.yAccum -= 1), i.yAccum < v && (i.yAccum = v), (w = !0), (S = 1)),
        !w &&
          i.yAccum <= -(1 + k) &&
          (this.attemptMicroRebuildY(i, r, -1, C) ||
            this.retreatLeafTeleport(i, r)) &&
          ((i.yAccum += 1),
          i.yAccum > -v && (i.yAccum = -v),
          (w = !0),
          (S = -1)),
        (i.cooldownX = Math.max(0, i.cooldownX - 1)),
        (i.cooldownY = Math.max(0, i.cooldownY - 1)));
      ...
      for (
        ...
        U < i.cellCount &&
        !(
          (s >= this.instanceCapacity &&
            (this.ensureInstanceStorage(s + 1), s >= this.instanceCapacity)) ||
          s >= this.maxInstances
        );
        U++
      ) {
        ...
        const H = s * 4;
        ((this.colors[H] = z[G]),
          (this.colors[H + 1] = z[G + 1]),
          (this.colors[H + 2] = z[G + 2]),
          (this.colors[H + 3] = $),
          s++);
      }
    }
    ...
    if (
      this.instanceCount > 0 &&
      (this.rotateInstanceBuffer(),
      this.gridBuffers.length > 0 && this.colBuffers.length > 0)
    ) {
      ...
      (o.bindBuffer(o.ARRAY_BUFFER, n),
        o.bufferSubData(
          o.ARRAY_BUFFER,
          0,
          this.gridCoords.subarray(0, this.instanceCount * 2),
        ),
        o.bindBuffer(o.ARRAY_BUFFER, i),
        o.bufferSubData(
          o.ARRAY_BUFFER,
          0,
          this.colors.subarray(0, this.instanceCount * 4),
        ));
    }
    this.bucketDirty &&
      this.bucketRebuildsThisFrame === 0 &&
  ...
  render() {
    ...
    (t.useProgram(this.program),
      ...
      this.instanceCount > 0 &&
        t.drawArraysInstanced(t.TRIANGLE_STRIP, 0, 4, this.instanceCount),
  ...
  initGL() {
    ...
    if (
      ...
      throw new Error(t.getShaderInfoLog(e) || "vs");
    ...
    if (
      ...
      throw new Error(t.getShaderInfoLog(s) || "fs");
    if (
      ...
      throw new Error(t.getProgramInfoLog(this.program) || "link");
    ((this.uCellSizeLoc = t.getUniformLocation(this.program, "uCellSize")),
      (this.uOffsetLoc = t.getUniformLocation(this.program, "uOffset")));
    const o = new Float32Array([-0.5, -0.5, 0.5, -0.5, -0.5, 0.5, 0.5, 0.5]);
    ((this.quadBuffer = t.createBuffer()),
      t.bindBuffer(t.ARRAY_BUFFER, this.quadBuffer),
      t.bufferData(t.ARRAY_BUFFER, o, t.STATIC_DRAW),
      (this.vao = t.createVertexArray()),
      t.bindVertexArray(this.vao),
      (this.posAttribLoc = t.getAttribLocation(this.program, "aPosition")),
      (this.gridAttribLoc = t.getAttribLocation(this.program, "aGridCoord")),
      (this.colAttribLoc = t.getAttribLocation(this.program, "aInstanceColor")),
      t.bindBuffer(t.ARRAY_BUFFER, this.quadBuffer),
      t.enableVertexAttribArray(this.posAttribLoc),
      t.vertexAttribPointer(this.posAttribLoc, 2, t.FLOAT, !1, 0, 0),
      t.bindVertexArray(null),
  ...
  trySpawnOne() {
    for (let e = 0; e < 32; e++) {
      const s = this.introSpawnOutside,
        o = s
          ? this.cols + ((Math.random() * this.introMarginX) | 0)
          : this.cols - 1 - ((Math.random() * this.spawnBandX) | 0),
        n = s
          ? -1 - ((Math.random() * this.introMarginY) | 0)
          : (Math.random() * this.spawnBandY) | 0,
        i = this.makeLeaf(o, n, s);
      if (this.canPlaceLeaf(i) && this.farFromOthers(i, this.minSpawnGridGap)) {
        (this.leaves.push(i),
          this.markCooldownsForLeaf(i),
          this.markSpatialIndexDirty());
        break;
      }
    }
  ...
  seedAgedField(t = 10) {
    ...
    for (; e.length < s && o-- > 0;) {
      const n = Math.random() < 0.35,
        i = n
          ? (this.cols + Math.random() * this.introMarginX) | 0
          : (Math.random() * (this.cols - 2)) | 0,
        r = n
          ? -1 - ((Math.random() * this.introMarginY) | 0)
          : (Math.random() * (this.rows - 2)) | 0,
        c = this.makeLeaf(i, r, n);
      ((c.age = Math.random() * Math.max(2, t)),
        (c.xAccum = (Math.random() * 2 - 1) * 0.99),
        (c.yAccum = (Math.random() * 2 - 1) * 0.99),
        this.farFromOthersLocal(c, e, this.minSpawnGridGap) && e.push(c));
    }
  ...
  makeLeaf(t, e, s) {
    const o = this.randRange(this.vxRange[0], this.vxRange[1]),
      ...
      r = this.windSineAmp * (0.6 + Math.random() * 0.8),
    ...
    for (let m = 1; m < a; m++) {
      ...
      for (let g = 0; g < 16 && !w; g++) {
        const [S, k] = f[(Math.random() * f.length) | 0],
          v = this.pickShapeDirections();
        for (let I = 0; I < v.length && !w; I++) {
          ...
          d.has(_) ||
            ...
            (l[m * 2 + 1] = y),
            y > u && (u = y),
            (Y > 0 || Math.random() < this.shapeBranchChance) && f.push([L, y]),
            (w = !0));
        }
      }
      if (!w) {
    ...
    for (let m = 0; m < a; m++) {
      const w = this.palette[(Math.random() * this.palette.length) | 0],
        g = m * 4;
      if (
        ((p[g + 0] = (this.clamp01(w[0]) * 255) | 0),
        (p[g + 1] = (this.clamp01(w[1]) * 255) | 0),
        (p[g + 2] = (this.clamp01(w[2]) * 255) | 0),
        (p[g + 3] = (this.clamp01(w[3]) * 255) | 0),
        C > 0)
      ) {
        const S = l[m * 2 + 1],
          k = this.clamp01(S / C);
  ...
  reseed(t) {
    for (let o = 0; o < 24; o++) {
      ...
      if (this.farFromOthers(r, this.minSpawnGridGap, t)) {
        ((this.leaves[t] = r),
          this.markCooldownsForLeaf(r),
          this.markSpatialIndexDirty());
        return;
      }
    }
    const s = this.makeLeaf(
      this.cols + this.introMarginX,
      -1 - this.introMarginY,
      !0,
    );
    ((this.leaves[t] = s),
      this.markCooldownsForLeaf(s),
      this.markSpatialIndexDirty());
  }
  farFromOthers(t, e, s = null) {
    const o = Math.max(1, e);
    if (this.leaves.length === 0) return !0;
    if ((this.ensureSpatialIndex(), !this.bucketValid))
      return this.farFromOthersFallback(t, e, s);
    const n = t.col,
      i = t.row,
      r = t.cells;
  ...
  canPlaceLeaf(t) {
    ...
    for (let r = 0; r < t.cellCount; r++) {
      const c = n + o[r * 2],
        a = i + o[r * 2 + 1];
      if (
        (e && a >= 0 && a < this.rows && this.rowCd[a] > 0) ||
        (s && c >= 0 && c < this.cols && this.colCd[c] > 0)
      )
        return !1;
    }
    return !0;
  ...
  markCooldownsForLeaf(t) {
    ...
    for (let r = 0; r < t.cellCount; r++) {
      const c = n + o[r * 2],
        a = i + o[r * 2 + 1];
      (e && a >= 0 && a < this.rows && (this.rowCd[a] = this.rowCooldownLen),
        s && c >= 0 && c < this.cols && (this.colCd[c] = this.colCooldownLen));
    }
  }
  pickShapeDirections() {
    const t = [
  ...
  computeLeafBounds(t) {
    if (!t._boundsDirty && t._minRow !== void 0)
      return {
        minRow: t._minRow,
        maxRow: t._maxRow,
        minCol: t._minCol,
    ...
    return (
      isFinite(e) || ((e = s = t.row), (o = n = t.col)),
      (t._minRow = e),
      (t._maxRow = s),
      (t._minCol = o),
      (t._maxCol = n),
      (t._boundsDirty = !1),
      { minRow: e, maxRow: s, minCol: o, maxCol: n }
    );
  }
  markBoundsDirty(t) {
    t._boundsDirty = !0;
  ...
  teleportLeafHorizontal(t, e, s) {
    ...
    if (t.cellCount === 1) {
      const a = t.cells[0] + o,
        l = t.cells[1];
      return this.canPlaceCellAtOffset(t, e, a, l)
        ? ((t.cells[0] = a),
          this.markBoundsDirty(t),
          this.normalizeLeafOffsets(t),
          this.markSpatialIndexDirty(),
    ...
    for (let a = 0; a < c; a++) {
      ...
      if (
        this.hasNeighborForOffset(t, n, l, u) &&
        this.canPlaceCellAtOffset(t, e, l, u, n)
      )
        return (
          (t.cells[n * 2] = l),
          (t.cells[n * 2 + 1] = u),
  ...
  advanceLeafTeleport(t, e) {
    ...
    if (t.cellCount === 1) {
      const r = t.cells[0],
        c = t.cells[1] + 1;
      return this.canPlaceCellAtOffset(t, e, r, c)
        ? ((t.cells[1] = c),
          this.markBoundsDirty(t),
          this.normalizeLeafOffsets(t),
          this.markSpatialIndexDirty(),
    ...
    for (let r = 0; r < i; r++) {
      ...
      if (
        this.hasNeighborForOffset(t, s, c, a) &&
        this.canPlaceCellAtOffset(t, e, c, a, s)
      )
        return (
          (t.cells[s * 2] = c),
          (t.cells[s * 2 + 1] = a),
  ...
  nextLeafRand(t) {
    return (
      (t.rngState = (1664525 * t.rngState + 1013904223) | 0),
      (t.rngState >>> 0) * 23283064365386963e-26
    );
  }
...
const X = 1,
  ...
  pt = st({
    ...
    props: {
      ...
      rows: { default: 20 },
      count: { default: 16 },
      cellMoveSpeedX: { default: 4.5 },
      cellMoveSpeedY: { default: 4.4 },
      prewarmSeconds: { default: 13 },
      minSegments: { default: 3 },
      maxSegments: { default: 3 },
      dprCap: { default: 1 },
    },
    setup(B) {
      const t = B,
        ...
        w = (b, M) => {
          ...
          R &&
            ...
            (s = new ft(R, {
              ...
              palette: [
                [0.9529, 0.302, 0.2078, 1],
                [0.9098, 0.2902, 0.2039, 1],
                [1, 0.3098, 0.2118, 1],
                [1, 0.2902, 0.1882, 1],
                [0.9608, 0.2863, 0.1882, 1],
              ],
            })),
            (x = b),
            s.seedAgedField(10),
            s.setCellMoveSpeed(d.value, f.value),
            s.resize(),
            s.clear(),
            s.render(),
            a &&
              ((L = T.getInstance().scrollPosition),
              n && !document.hidden && k()));
        },
        g = (b = !1) => {
          if (C) {
      ...
      const _ = (b) => {
        ...
        const A = f.value,
          ...
          E = c(-R, -O, O);
        (s.setCellMoveSpeed(d.value, E * 2),
          (y = window.setTimeout(() => {
      ...
      return (
        ot(() => {
          ...
          !document.hidden
            ? ((n = !0), k())
            : (s == null || s.clear(), s == null || s.render());
        }),
        j(
          () => l.value,
          () => {
        ...
        (b, M) => (
          rt(),
          at("canvas", { ref_key: "sceneCanvas", ref: e }, null, 512)
        )
      );
    },
  });
const xt = lt(pt, [["__scopeId", "data-v-950bfb3c"]]);
export { xt as _ };
